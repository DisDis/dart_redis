part of metrics;

//var EDS = require('../stats/exponentially_decaying_sample')
//  , UniformSample = require('../stats/uniform_sample');

var DEFAULT_PERCENTILES = [0.001, 0.01, 0.05, 0.1, 0.25, 0.5, 0.75, 0.9, 0.95, 0.98, 0.99, 0.999];

class PrintObj {
  var type;
  var min;
  var max;
  var sum;
  var variance;
  double mean;
  var std_dev;
  var count;
  var median;
  var p75;
  double p95;
  var p99;
  var p999;
  PrintObj({this.type, this.min, this.max, this.sum, this.variance, this.mean, this.std_dev, this.count, this.median, this.p75, this.p95, this.p99, this.p999});
}

/*
* A histogram tracks the distribution of items, given a sample type 
*/
class Histogram {
  ExponentiallyDecayingSample sample;
  int min;
  int max;
  int sum;
  int count;
  String type;
  double varianceM;
  double varianceS;

  Histogram([sample]) {
    if (sample == null) {
      this.sample = new ExponentiallyDecayingSample(1028, 0.015);
    } else {
      this.sample = sample;
    }
    this.min = null;
    this.max = null;
    this.sum = null;
    // These are for the Welford algorithm for calculating running variance
    // without floating-point doom.
    /*this.varianceM = null;
    this.varianceS = null;
    this.count = 0;*/
    clear();
    this.type = 'histogram';
  }

  clear() {
    this.sample.clear();
    this.min = null;
    this.max = null;
    this.sum = null;
    this.varianceM = 0.0;
    this.varianceS = 0.0;
    this.count = 0;
  }

  // timestamp param primarily used for testing
  update(int val, [timestamp]) {
    this.count++;
    this.sample.update(val, timestamp);
    if (this.max == null) {
      this.max = val;
    } else {
      this.max = val > this.max ? val : this.max;
    }
    if (this.min == null) {
      this.min = val;
    } else {
      this.min = val < this.min ? val : this.min;
    }
    this.sum = (this.sum == null) ? val : this.sum + val;
    this.updateVariance(val);
  }

  updateVariance(int val) {
    var oldVM = this.varianceM;
    var oldVS = this.varianceS;
    if (this.count == 1) {
      this.varianceM = val.toDouble();
    } else {
      this.varianceM = oldVM + (val - oldVM) / this.count;
      this.varianceS = oldVS + (val - oldVM) * (val - this.varianceM);
    }
  }

  // Pass an array of percentiles, e.g. [0.5, 0.75, 0.9, 0.99]
  percentiles([percentiles]) {
    if (percentiles == null) {
      percentiles = DEFAULT_PERCENTILES;
    }
    var values = this.sample.getValues();/*new List.from(this.sample.getValues().map((v) {
      return v;//parse
    }));*/
    values.sort((a, b) {
      return a - b;
    });
      var  scores = {},
        percentile,
        pos,
        lower,
        upper;
    for (var i = 0; i < percentiles.length; i++) {
      pos = percentiles[i] * (values.length + 1);
      percentile = percentiles[i];
      if (pos < 1) {
        scores[percentile] = values[0];
      } else if (pos >= values.length) {
        scores[percentile] = values[values.length - 1];
      } else {
        lower = values[(pos.floor()) - 1];
        upper = values[(pos.ceil()) - 1];
        scores[percentile] = lower + (pos - pos.floor()) * (upper - lower);
      }
    }
    return scores;
  }

  variance() {
    return this.count < 1 ? null : this.varianceS / (this.count - 1);
  }

  mean() {
    return this.count == 0 ? null : this.varianceM;
  }

  stdDev() {
    return this.count < 1 ? null : Math.sqrt(this.variance());
  }

  values() {
    return this.sample.getValues();
  }

  PrintObj printObj() {
    var percentiles = this.percentiles();
    return new PrintObj(type: 'histogram', min: this.min, max: this.max, sum: this.sum, variance: this.variance(),
        mean: this.mean(), std_dev: this.stdDev(), count: this.count, 
        median: percentiles[0.5], p75: percentiles[0.75], p95: percentiles[0.95], p99: percentiles[0.99], p999: percentiles[0.999]);
  }
}
//module.exports.createExponentialDecayHistogram(size, alpha) { return new Histogram(new EDS((size || 1028), (alpha || 0.015))); };
//module.exports.createUniformHistogram(size) { return new Histogram(new UniformSample((size || 1028))); };
//module.exports.DEFAULT_PERCENTILES = DEFAULT_PERCENTILES;
